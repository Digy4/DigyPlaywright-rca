 const { Given, Then } = require('@cucumber/cucumber');
         ...
         Given('user has provided {string}, {string}, {string}, {string}, {string} in digy4.config.properties file',{ timeout: 20000 }, async function (projectName, framework, suiteName, release, testType) {
         ...
         Then('user is validating {string},{string},{string},{string},{string},{string} records',async function (total,pass,fail,duration,startTime,endTime) {
         ...
         Then('it should show as per execution performed by user',async function () {
            await this.page.getByRole('paragraph', { state: 'visible' }).filter({ hasText: /^Test Suite$/ }).click();
         });
